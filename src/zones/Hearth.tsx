...
function Anvil() {
  ...
  return (
    <group>
      ...
      {/* Horn */}
      <mesh position={[1.8, 1.4, 0]} geometry={hornGeo} material={brightMetal} castShadow />
    </group>
  );
}
...
function FirePit() {
  ...
  const coalSizes = useMemo(() => Array.from({ length: 12 }, () => 0.1 + Math.random() * 0.12), []);
  ...
  return (
    <>
      {pillars.map((p, i) => (
        <group key={i}>
          {/* Pillar */}
          <mesh position={[p.x, 1.5, p.z]} geometry={pillarGeo} material={darkMetal} castShadow />
          {/* Torch */}
          <mesh position={[p.x, 3.2, p.z]} geometry={torchGeo} material={torchMaterial} />
          {/* Torch light */}
          <pointLight
            color={0xff6b1a}
            intensity={0.5}
...
export const Hearth = memo(function Hearth() {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireLightRef.current) {
      fireLightRef.current.intensity = 3.5 + Math.sin(t * 8) * 0.6 + Math.sin(t * 13) * 0.35;
      fireLightRef.current.position.y = 2.5 + Math.sin(t * 3) * 0.1;
    }
